...
function setTasks(tasks){ localStorage.setItem(TASK_KEY, JSON.stringify(tasks)); }
...
function initTasks(){
  // いったん全部消す（重複描画を防止）
  document.querySelectorAll('.task-item').forEach(n => n.remove());

  const saved = getTasks();
  if (saved.length === 0) {
    // 初回だけデフォルト1件を作成して保存
    renderTask('New Task', false);
    setTasks([{ text:'New Task', completed:false }]);
  } else {
    saved.slice(0,3).forEach(t => renderTask(t.text, t.completed));
  }
}

document.addEventListener('DOMContentLoaded', initTasks, { once:true });
